...
export default function OrderTable() {
  ...
  const { data, error, isLoading } = useAccessData<PaginationModel<OrderModel>>(
    `/order?page=${page}&pageSize=${sizePage}${
  ...
  return (
    <>
      {(!data || isLoading) && <LoaderModal open={true} />}
      <Sheet
          ...
          <tbody>
            {dataOrder?.rows?.map((row) => (
              ...
              >
                ...
                <td>
                  <Typography level='body-xs'>{row.orderCode}</Typography>
                </td>
                <td>
                ...
                <td>
                  <Chip
                    ...
                    startDecorator={
                      {
                        ["Waiting Confirmation"]: <PendingActionsRounded />,
                        ["In Process"]: (
                          <Icon
      ...
      {(dataOrder?.pagination.total ?? 1) > 1 && (
        <Box
          // className='Pagination-tabletUp'
          sx={{ display: "flex", justifyContent: "space-around" }}
        >
          <Pagination
            count={dataOrder?.pagination.total ?? 1}
            page={dataOrder?.pagination.current ?? 1}
            onChange={(e, page) => {
              setPage(page);
            }}
            color='primary'
